...
def convert_rule_to_short_name(rule_name: str) -> str:
    ...
    if rule_name is None:
        return rule_name
    if isinstance(rule_name, float) and (rule_name != rule_name):  # Check for NaN
        return rule_name
    
    # Convert to string and handle pandas NaN string representation
    rule_name = str(rule_name).strip()
    if not rule_name or rule_name.lower() == "nan" or rule_name == "N/A":
        return rule_name
    
    # Normalize the input: lowercase and remove all spaces
    rule_normalized = rule_name.lower().replace(' ', '').replace('-', '')
    ...
    # Create a normalized lookup dictionary (all keys lowercase, no spaces)
    normalized_mapping = {}
    for full_name, short_name in rule_mapping.items():
        # Normalize key: lowercase and remove all spaces
        normalized_key = full_name.lower().replace(' ', '').replace('-', '')
        # Keep the first occurrence (most specific) if there are duplicates
        if normalized_key not in normalized_mapping:
            normalized_mapping[normalized_key] = short_name
    
    # Try exact match first
    if rule_normalized in normalized_mapping:
        return normalized_mapping[rule_normalized]
    
    # Try partial match (e.g., "modusponensrule" -> "modusponens")
    # Sort by length (longest first) to match more specific rules first
    sorted_mappings = sorted(normalized_mapping.items(), key=lambda x: len(x[0]), reverse=True)
    for normalized_key, short_name in sorted_mappings:
        if normalized_key in rule_normalized:
            return short_name
...
def main():
    ...
    input_path = 'Data/llm_output/gpt/gpt_baseline_1.jsonl'
    record_number = None
    all_rows = []
    with open(input_path, 'r') as f:
        ...
        print(df.shape)
        df.to_csv("Data/csv/gpt/gpt_baseline_1.csv", index=False)
