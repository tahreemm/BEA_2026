...
def main():
    ...
    input_path = 'Data/llm_output/gpt_ours.jsonl'
    record_number = None
    all_rows = []
    with open(input_path, 'r') as f:
        ...
        for line in f:
            ...
            student_response = obj.get("student_response")
            if student_response is None or not isinstance(student_response, dict):
                student_response = {}
            student_plan = student_response.get("REASONING", None) 
            print("student plan: ", student_plan)
            student_step = student_response.get("NEXT_STEP", None)
            print("student step: ", student_step)
            student_rule = student_response.get("RULE", None)
            ...
            # teacher response
            teacher_response = obj.get("teacher_response", "") or {}
            teacher_plan = teacher_response.get("TEACHER_PLAN") or None
            print("teacher plan: ", teacher_plan)
            teacher_rule = teacher_response.get("TEACHER_RULE") or None
            print("teacher rule: ", teacher_rule)
            teacher_feedback = teacher_response.get("TEACHER_FEEDBACK") or None
            print("teacher feedback: ", teacher_feedback)
            next_step_correctness_teacher = teacher_response.get("NEXT_STEP_CORRECTNESS") or None
            print("next step correctness teacher: ", next_step_correctness_teacher)
            ...
            teacher_parent_statements = [parent.strip() for parent in teacher_parent_statements] or []
            print("teacher parent statements: ", teacher_parent_statements)
            teacher_parent_complexities = [logical_complexity(parent) for parent in teacher_parent_statements] or []
            print("teacher parent complexities: ", teacher_parent_complexities)
            
            row_data["teacher_plan"] = teacher_plan
            ...
            row_data["next_step_correctness_teacher"] = next_step_correctness_teacher
            ...
            next_step_correctness_judge = judge_response.get("NEXT_STEP_CORRECTNESS") or None
            print("next step correctness judge: ", next_step_correctness_judge)
            judge_feedback = judge_response.get("FINAL_FEEDBACK") or None
            print("judge feedback: ", judge_feedback)
            row_data["next_step_correctness_judge"] = next_step_correctness_judge
            row_data["judge_feedback"] = judge_feedback
            row_data["judge_completion_tokens"] = 0  #[completion_tokens[2]] 
            row_data["judge_completion_time"] = 0  #[completion_time[2]]   


            student_update_response = obj.get("student_update_response")
            if student_update_response is None or not isinstance(student_update_response, dict):
                student_update_response = {}
            student_update_plan = student_update_response.get("IMPROVED_STEP") or None
            print("student update plan: ", student_update_plan)
            student_update_rule = student_update_response.get("BETTER_RULE") or None
            print("student update rule: ", student_update_rule)
            student_update_revised_plan = student_update_response.get("REVISED_REASONING") or None
        ...
        print(df.shape)
        df.to_csv("Data/csv/gpt_ours.csv", index=False)
