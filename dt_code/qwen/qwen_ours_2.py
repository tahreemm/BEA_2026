...
from dt_code.qwen.qwen_interface import make_student_call, make_teacher_only_call, make_judge_only_call, make_judge_verifier_call, make_student_update_call
...
def append_output_line(outfile, id, problem_number, stepPreState, givens, conclusion, intermediates, correct_step, student_response, known_expressions, judge_response, judge_response_2, student_update_response, cleaned_kg_final_steps, completion_tokens, total_tokens, time_taken):
    try:
...
def main():
    ...
    input_path = 'Data/llm_output/qwen/qwen_baseline_2.jsonl'
    output_path = 'Data/llm_output/qwen/qwen_ours_2.jsonl'
    ...
    with open(input_path, 'r') as infile:
        ...
        for line in infile:
            ...
            id = data.get('id', '')
            print("id: ", id)
            problem_number = data.get("problem number", data.get("problem_number", ""))
            ...
            judge_feedback_message = f"""NEXT_STEP_CORRECTNESS: {next_step_correctness}
            ...
            judge_response_raw, completion_tokens_judge, total_tokens_judge, time_taken_judge = make_judge_verifier_call(givens, conclusion, intermediates, student_response, judge_feedback_message, cleaned_kg_final_steps)
            judge_response_2 = extract_json_object(judge_response_raw)
            print("--------------------------------")
            print("Judge Response:")
            print(judge_response_2)
            print("--------------------------------")
            if judge_response_2 is None:
            ...
            final_feedback = judge_response_2.get('FINAL_FEEDBACK', '')
            student_errors = judge_response_2.get('STUDENT_ERRORS', '')
            ...
            append_output_line(output_path, id, problem_number, stepPreState, givens, conclusion, intermediates, correct_step, student_response, known_expressions, judge_response, judge_response_2, student_update_response, cleaned_kg_final_steps, completion_tokens, total_tokens, time_taken)
