...
def process_all_instances(
    ...
    for idx, (instance_tutor, instance_teacher, instance_judge) in enumerate(tqdm(
        ...
        judge_resp_raw = instance_judge.get('judge__response', {})
    ...
    tutor_texts = [parse_response(inst.get("tutor_response", {}), "TUTOR_FEEDBACK") or
                   parse_response(inst.get("tutor_response", {}), "TEACHER_FEEDBACK")
                   for inst in data_tutor]
    teacher_texts = [parse_response(inst.get("teacher_response", {}), "TEACHER_FEEDBACK") 
                     for inst in data_teacher]
    judge_texts = [parse_response(inst.get("judge_response", {}), "FINAL_FEEDBACK") or 
...
if __name__ == "__main__":
    ...
    process_all_instances(
        data_path_TUTOR='Data/llm_output/mistral/mistral_baseline_1.jsonl',      # S→Tu
        data_path_TEACHER='Data/llm_output/mistral/mistral_baseline_2.jsonl',  # S→T
        data_path_JUDGE='Data/llm_output/mistral/mistral_ours.jsonl',      # S→Tu→J
        output_path='Data/llm_output/mistral/metrics_results.csv',
        use_bertscore=True,
        device='cpu'
    )