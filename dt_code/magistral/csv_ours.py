...
def main():
    ...
    input_path = 'Data/llm_output/magistral/magistral_ours.jsonl'
    record_number = None
    all_rows = []
    with open(input_path, 'r') as f:
        ...
        for line in f:
            ...
            judge_response = obj.get("judge_response", "") or {}
            student_errors_ours = judge_response.get("STUDENT_ERRORS") or None
            print("student errors ours: ", student_errors_ours)
            next_step_correctness_ours = judge_response.get("NEXT_STEP_CORRECTNESS") or None
            print("next step correctness ours: ", next_step_correctness_ours)
            teacher_feedback_correctness_ours = judge_response.get("TEACHER_FEEDBACK_CORRECTNESS") or None
            print("teacher feedback correctness ours: ", teacher_feedback_correctness_ours)
            teacher_rule_correctness_ours = judge_response.get("TEACHER_RULE_CORRECTNESS") or None
            print("teacher rule correctness ours: ", teacher_rule_correctness_ours)
            judge_action_ours = judge_response.get("JUDGE_ACTION") or None
            print("judge action ours: ", judge_action_ours)
            final_feedback_ours = judge_response.get("FINAL_FEEDBACK") or None
            print("final feedback ours: ", final_feedback_ours)
 
            row_data["student_errors_ours"] = student_errors_ours
            row_data["next_step_correctness_ours"] = next_step_correctness_ours
            row_data["teacher_feedback_correctness_ours"] = teacher_feedback_correctness_ours
            row_data["teacher_rule_correctness_ours"] = teacher_rule_correctness_ours
            row_data["judge_action_ours"] = judge_action_ours
            row_data["final_feedback_ours"] = final_feedback_ours
            row_data["ours_completion_tokens"] = [completion_tokens[0]] 
            row_data["ours_completion_time"] = [completion_time[0]]   
            ...
            row_data["ours_student_update_step"] = student_update_step
            row_data["ours_student_update_rule"] = student_update_rule
            row_data["ours_student_update_revised_reasoning"] = student_update_revised_reasoning
            row_data["ours_student_update_parent_statements"] = student_update_parent_statements
            row_data["ours_student_update_completion_tokens"] = [completion_tokens[1]] 
            print("ours student update completion tokens: ", completion_tokens[1])
            row_data["ours_student_update_completion_time"] = [completion_time[1]]   
        ...
        print(df.shape)
        df.to_csv("Data/csv/magistral/magistral_ours.csv", index=False)
