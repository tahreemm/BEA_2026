...
def main():
    ...
    with open(input_path, 'r') as f:
        ...
        for line in f:
            ...
            judge_student_errors = judge_response.get("STUDENT_ERRORS")
            print("judge student errors: ", judge_student_errors)
            ...
            row_data["judge_student_errors"] = judge_student_errors
            row_data["judge_completion_tokens"] = [completion_tokens[0] if len(completion_tokens) > 0 else ""] 
            row_data["judge_completion_time"] = [completion_time[0] if len(completion_time) > 0 else ""]   
            ...
            row_data["judge_student_update_step"] = student_update_step
            row_data["judge_student_update_rule"] = student_update_rule
            row_data["judge_student_update_revised_reasoning"] = student_update_revised_reasoning
            row_data["judge_student_update_parent_statements"] = student_update_parent_statements
            row_data["judge_student_update_completion_tokens"] = [completion_tokens[1] if len(completion_tokens) > 1 else ""] 
            print("judge student update completion tokens: ", completion_tokens[1] if len(completion_tokens) > 1 else "")
            row_data["judge_student_update_completion_time"] = [completion_time[1] if len(completion_time) > 1 else ""]   
            print("judge student update completion time: ", completion_time[1] if len(completion_time) > 1 else "")
    ...
    print(df.shape)
    output_path = "Data/csv/llama3/llama_baseline_2.csv"
